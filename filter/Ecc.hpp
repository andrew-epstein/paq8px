...
struct Ecc {
    ...
    void
    eccCompute(const uint8_t *src, uint32_t majorCount, uint32_t minorCount, uint32_t majorMult, uint32_t minorInc, uint8_t *dest) const {
    ...
    uint32_t edcCompute(const uint8_t *src, int size) const {
      uint32_t edc = 0;
      while((size--) != 0 ) {
