...
#define LZW_FIND(k) \
  { \
    ...
    int stride = (offset > 0) ? lzwTableSize - offset : 1; \
    while (true) { \
...
#define LZW_RESET \
  { \
    for (int i = 0; i < lzwTableSize; table[i] = -1, i++) \
      ; \
  }
...
private:
    static constexpr int lzwTableSize = 9221;
public:
    int encode(File *in, File *out, uint64_t len, int info, int &headerSize) override {
